...
const PORT_MENU = import.meta.env.VITE_MENU_PORT || 5173;
  ...
  return (
    ...
    >
      ...
      <div className="px-4 mb-2">
        <a
          href={`http://localhost:${PORT_MENU}`}
          target="_blank"
          rel="noopener noreferrer"
          className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-200 group hover:bg-cm-accent/5 ${
            isCollapsed ? "justify-center" : ""
          }`}
        >
          <ExternalLink size={20} className="shrink-0 text-cm-text-secondary group-hover:text-cm-accent transition-colors" />
          {!isCollapsed && (
            <motion.span
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              className="whitespace-nowrap overflow-hidden text-sm tracking-tight text-cm-text-secondary group-hover:text-cm-text"
            >
              Ir a Menu →
            </motion.span>
          )}
