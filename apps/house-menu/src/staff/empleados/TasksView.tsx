...
import { subscribeGoals } from './employeeService';
            ...
            pending.map((goal, i) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-cm-text">{(goal.title as string) || 'Sin título'}</div>
                  {goal.description && (
                    <div className="text-xs text-cm-text-secondary mt-0.5">{goal.description as string}</div>
                  )}
                  {goal.dueDate && (
                    <div className="text-[0.65rem] text-cm-warning mt-1 font-medium">
                      Vence: {new Date(goal.dueDate as string).toLocaleDateString('es-PE')}
                    </div>
                  )}
                </div>
              </motion.div>
          ...
          <div className="space-y-1.5">
            {completed.map((goal) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="text-sm text-cm-text line-through">{goal.title as string}</div>
                </div>
              </div>
            ))}
          </div>
