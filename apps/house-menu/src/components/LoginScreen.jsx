  ...
  return (
    <div className="min-h-screen bg-cm-bg flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Dev users: quick-access buttons */}
          ...
          <div className="space-y-1.5">
            {devUsers.map((u) => (
              <button
                key={u.id}
                type="button"
                onClick={() => { setEmail(u.email); setPin(u.pin); onLogin(u.email, u.pin); }}
                disabled={isLoading}
                className="w-full flex items-center justify-between px-4 py-2.5 border border-cm-border rounded-lg text-left hover:bg-cm-bg-alt hover:border-cm-accent/30 transition-colors disabled:opacity-50"
              >
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-cm-text truncate">{u.name}</p>
                  <p className="text-[0.6rem] text-cm-muted font-medium truncate">{u.role}</p>
                </div>
                <span className="shrink-0 text-[0.55rem] font-mono font-bold text-cm-text-tertiary bg-cm-bg-alt px-2 py-0.5 rounded">
                  {u.pin}
                </span>
              </button>
            ))}
        ...
        {(devUsers?.length > 0 || (onGoogleLogin && firebaseReady)) && (
