  ...
  return (
    <div className="flex items-center gap-2 bg-cm-bg/50 rounded-lg p-2 border border-cm-border group hover:border-cm-border transition-colors">
      ...
      <input
        ...
        className="flex-1 px-2 py-1 rounded-lg border border-cm-border bg-white text-xs font-bold text-cm-text focus:outline-none focus:border-cm-accent placeholder:text-cm-muted"
      />
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <input
          ...
          className="w-14 px-2 py-1 rounded-lg border border-cm-border bg-white text-xs font-black text-cm-accent text-right focus:outline-none focus:border-cm-accent"
        />
      </div>
      ...
      {opt.trackStock ? (
        <div className="flex items-center gap-1 shrink-0">
          ...
          <input
            ...
            className="w-10 px-1 py-1 rounded-lg border border-cm-border bg-white text-xs font-bold text-cm-text text-center focus:outline-none focus:border-cm-accent"
          />
          <button
        ...
        <button
          onClick={() => onChange({ ...opt, trackStock: true, stock: 10 })}
          className="text-[9px] font-bold text-cm-muted hover:text-cm-accent bg-white hover:bg-indigo-50 px-1.5 py-1 rounded-lg border border-cm-border hover:border-indigo-300 transition-all shrink-0 uppercase tracking-wider"
...
function StepEditor({ step, index, onChange, onRemove }) {
  ...
  const addOption = () => {
    const newOpt = { id: uid('opt'), name: '', price: 0, icon: '📍', trackStock: false, stock: 0 };
    ...
    <div className="bg-white rounded-xl border border-indigo-200 shadow-sm">
      <div className="flex items-center gap-2 p-3 bg-indigo-50/50 border-b border-indigo-100 rounded-t-xl">
        ...
        <input
          ...
          className="flex-1 px-2 py-1 rounded-lg border border-indigo-200 bg-white text-xs font-bold text-cm-text focus:outline-none focus:border-indigo-400 placeholder:text-cm-muted"
        />
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[9px] font-bold text-cm-muted uppercase">Tipo</span>
          <button
            onClick={() => onChange({ ...step, type: step.type === 'single' ? 'multiple' : step.type === 'multiple' ? 'auto' : 'single' })}
...
export default function WizardConfigModal({ open, product, onSave, onClose }) {
  ...
  const addStep = () => {
    setSteps([...steps, { id: uid('step'), title: '', type: 'single', options: [] }]);
  };
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between p-4 bg-white border-b border-cm-border shrink-0">
              <div>
            ...
            <div className="flex items-center justify-between p-4 bg-white border-t border-cm-border shrink-0">
