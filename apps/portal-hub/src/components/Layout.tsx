...
import { Clock, CalendarDays, ListChecks, LogOut, User, Building2, History, Settings } from 'lucide-react';
import type { Employee, ViewId, NavItem } from '../types';
...
export default function Layout({ activeView, onNavigate, employee, branchId, onLogout, children }: LayoutProps) {
  const isAdmin = employee?.role && ADMIN_ROLES.includes(employee.role);
  const visibleItems = isAdmin ? NAV_ITEMS : NAV_ITEMS.filter(i => i.id !== 'admin');
  return (
      ...
      <div className="flex flex-col flex-1 min-w-0 md:pl-64">
        ...
        <header className="hidden md:flex h-14 items-center justify-between px-4 md:px-6 bg-cm-surface border-b border-cm-border shrink-0">
          <h1 className="text-base font-semibold text-cm-text">
            {visibleItems.find(i => i.id === activeView)?.label || 'Portal'}
          </h1>
          {employee && (
